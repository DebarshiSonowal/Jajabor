...
public class SpecialOrderActivity extends AppCompatActivity implements PaymentResultWithDataListener {
    ...
    protected void onStart() {
        super.onStart();
        mViewModel.getno().observe(this, new Observer<Integer>() {
            @Override
...
//                Toast.makeText(CartActivity.this,"Changexcx"+integer,Toast.LENGTH_SHORT).show();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mEmptyView = findViewById(R.id.progressActivity1);
...
//        }else
//            mEmptyView.showContent();
        Cursor cursor =  mDatabase.rawQuery("SELECT SUM(" + Contract3.CartItem.COLUMN_PRICE + ") as Total FROM " + Contract3.CartItem.TABLE_NAME, null);
            ...
            public void onClick(View v) {
                ...
                if (mUser != null) {
                    if (!TextUtils.isEmpty(Delivery.getText())) {
                        ...
                        FancyToast.makeText(SpecialOrderActivity.this,"Delivery Address is not available please add an address",FancyToast.LENGTH_SHORT,FancyToast.ERROR,false).show();
                        new BottomDialog.Builder(SpecialOrderActivity.this)
                                ...
                                .onPositive(new BottomDialog.ButtonCallback() {
                                    ...
                                    public void onClick(BottomDialog dialog) {
                                        ...
                                        Navigation.findNavController(SpecialOrderActivity.this, R.id.nav_host_fragment).navigate(R.id.nav_profile);
                                    }
                                }).show();
                    }
                } else {
                    ChocoBar.builder().setActivity(SpecialOrderActivity.this)
                            ...
                            .setActionClickListener(new View.OnClickListener() {
                                @Override
                                public void onClick(View v) {
                                    startActivity(new Intent(SpecialOrderActivity.this, login.class));
                                }
                            })
                            .red()
                            .show();
    ...
    private List<String> getColour(){
        ...
        Cursor res =  db.rawQuery( "select * from "+Contract3.CartItem.TABLE_NAME, null );
        res.moveToFirst();
        if (res != null)
        {
            while(res.isAfterLast() == false){
                arrayList.add(res.getString(res.getColumnIndex(Contract3.CartItem.COLUMN_COLOR)));
    ...
    private List<String>getPic(){
        ...
        {
            while(res.isAfterLast() == false){
                arrayList.add(res.getString(res.getColumnIndex(Contract3.CartItem.COLUMN_PIC)));
                Log.d("emailssinlisttt",arrayList.toString());
                res.moveToNext();
            }
        }
    ...
    private List<String> getsizes() {
        ...
        {
            while(res.isAfterLast() == false){
                arrayList.add(res.getString(res.getColumnIndex(Contract3.CartItem.COLUMN_SIZE)));
                Log.d("emailssinlisttt",arrayList.toString());
                res.moveToNext();
            }
        }
    ...
    private List<Integer> getquantity() {
        ...
        {
            while(res.isAfterLast() == false){
                arrayList.add(res.getInt(res.getColumnIndex(Contract3.CartItem.COLUMN_QUANTITY)));
                Log.d("emailssinlisttt",arrayList.toString());
                res.moveToNext();
            }}
        return arrayList;
...
//            Checkout.clearUserData(getContext());
    ...
    private void removeItem(long id) {
        mDatabase.delete(Contract3.CartItem.TABLE_NAME,
                Contract3.CartItem._ID + "=" + id, null);
...
//        itemno.setText("Item"+" ("+mCursor.getCount()+")");
    ...
    public ArrayList<Integer> getAllPid() {
        ...
        {
            while(res.isAfterLast() == false){
                arrayList.add(res.getInt(res.getColumnIndex(Contract3.CartItem.COLUMN_PID)));
                Log.d("emailssinlisttt",arrayList.toString());
                res.moveToNext();
            }}
        return arrayList;
    ...
    public void onPaymentSuccess(String s, PaymentData paymentData) {
        ...
        mDatabase.delete(Contract3.CartItem.TABLE_NAME,null,null);
        mProductAdapter.swapCursor(getAllItems());
        getMRP();
        try {
...
//        invoiceClient.create();
    ...
    private void addToPayment(String s,PaymentData paymentData) {
        ...
        db.collection("Payment").document(s).set(note).addOnSuccessListener(new OnSuccessListener<Void>() {
            ...
            public void onSuccess(Void aVoid) {
                Toast.makeText(SpecialOrderActivity.this, "Data stored ", Toast.LENGTH_SHORT).show();
            }
        });
    }
    ...
    private void addToOrder(PaymentData paymentData) {
        ...
        note.put("UID",FirebaseAuth.getInstance().getCurrentUser().getUid());
        ORDERid = db.collection("Special Order").document().getId();
        db.collection("Special Order").document(ORDERid).set(note).addOnSuccessListener(new OnSuccessListener<Void>() {
            @Override
            public void onSuccess(Void aVoid) {
                ChocoBar.builder().setActivity(SpecialOrderActivity.this)
